...
const MakeEdit = ({prev, onCLick} : {prev: string, onCLick: (content: string) => void}) => {
  ...
  return (
    <section className="flex justify-between items-center mt-5 relative">
        ...
        <textarea value={content}  onChange={(e) => setContent(e.target.value)} className="outline-1 outline-(--Grayish-Blue) w-full mx-5 p-2 rounded-lg" placeholder="Update Comment" name="mew-comment" id="new-comment"></textarea>
        <button onClick={() => {
            if(content.length == 0){
                setIsEmpty(true)
                return
